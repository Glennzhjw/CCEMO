...
void qSortDistance(int *distance, int left, int right){
	...
	if (left<right){
		...
		distance[index] = temp;
		pivot = repertoryDensity[distance[right]];
		i = left-1;
		for (j=left; j<right; j++){
			if (repertoryDensity[distance[j]] <= pivot){
				i+=1;
				temp = distance[j];
...
void qSortFrontObj(int objcount, int arrayFx[], int left, int right){
	...
	if (left<right){
		...
		arrayFx[index] = temp;
		pivot = repertFit[(arrayFx[right])*nObj+objcount];
		i = left-1;
		for (j=left; j<right; j++){
			if (repertFit[(arrayFx[j])*nObj+objcount] <= pivot){
				i+=1;
				temp = arrayFx[j];
				arrayFx[j] = arrayFx[i];
				arrayFx[i] = temp;
